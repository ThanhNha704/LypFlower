...
import { Search, ListOrdered, CalendarDays, Plus, X, Shield, ShieldAlert, User, ShieldCheck, Lock, Unlock } from 'lucide-react';
...
const ROLE_LABELS = {
  Admin: { label: 'Quản trị viên', cls: 'bg-rose-50 text-rose-600 border border-rose-100', icon: ShieldAlert },
  Staff: { label: 'Nhân viên', cls: 'bg-blue-50 text-blue-600 border border-blue-100', icon: Shield },
...
export default function AdminStaff() {
  ...
  const handleToggleActive = async (userId) => {
    try {
      ...
      toast.error(err.response?.data?.message || 'Không thể khóa/mở khóa tài khoản');
    }
  };
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {users.map(u => {
                ...
                const RoleIcon = ROLE_LABELS[u.role]?.icon || User;
                ...
                return (
                  <tr key={u.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-xl text-xs font-semibold ${roleStyle.cls}`}>
                        <RoleIcon size={12} />
                        {roleStyle.label}
                      </span>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <span className={`inline-flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-lg ${u.isActive ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                    ...
                    <td className="px-5 py-4">
                      {isMe ? (
                        <span className="text-xs text-gray-400 italic font-medium">Đang đăng nhập</span>
                      ) : (
                        <div className="flex items-center gap-2">
                          ...
                          <select 
                            ...
                            className="text-xs font-semibold border rounded-lg px-2 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-amber-500"
                          >
                          ...
                          <button
                            ...
                            className={`p-1.5 rounded-lg border transition-all ${u.isActive ? 'hover:bg-red-50 hover:text-red-600 text-gray-400 border-gray-200' : 'bg-red-50 text-red-600 border-red-200 hover:bg-white hover:text-gray-400'}`}
                            title={u.isActive ? "Khóa tài khoản" : "Mở khóa tài khoản"}
                          >
                            {u.isActive ? <Unlock size={14} /> : <Lock size={14} />}
                          </button>
                        </div>
                      )}
                    </td>
